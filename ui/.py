    ...
    for index, (key, value) in enumerate(obj.items()):
        ...
        if isinstance(value, str):
            ...
            if isinstance(value, dict):
                with gr.Row():
                    new_html = f'<li><details open><summary><div class="tree-div"><textarea class="tree-branch" id="parent-branch-{level}-{index}-{current_branch_id}" placeholder="{placeholders[str(level%3)]}">{key}</textarea></div></summary><ul>'
                    tree += f'{recursive_get_tree(value, level + 1, branch_id, new_html)}</ul></details></li>'
